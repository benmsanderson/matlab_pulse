...
if pct==1
    fprintf('%s%s',[repmat(sprintf('\b'),1,lastNchar),sprintf('\n')]);lastNchar=0;
    return
end
if (cputime-lasttime>0.1)
    ...
    %curmtxt=mat2str(curm);
            progressmsg=sprintf(['GWMCMC %5.1f%% [%s] %s %3.0f%% '],pct*100,progressmsg,ETA);
    ...
    if pct==0
       ...
       fprintf('%s%s',repmat(char(8),1,lastNchar),[reverseStr,progressmsg]);
